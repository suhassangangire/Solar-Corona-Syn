...
fitting_basis = 'per tc secosw sesinw k'    # Fitting basis, see Sun-Corona-Syn.basis.BASIS_NAMES for available basis names
bjd0 = 2454833.
planet_letters = {1: 'b', 2:'c'}

# Define prior centers (initial guesses) in a basis of your choice (need not be in the fitting basis)
anybasis_params = Sun-Corona-Syn.Parameters(nplanets,basis='per tc e w k')    # initialize Parameters object

anybasis_params['per1'] = Sun-Corona-Syn.Parameter(value=20.885258)    # period of 1st planet
anybasis_params['tc1'] = Sun-Corona-Syn.Parameter(value=2072.79438)    # time of inferior conjunction of 1st planet
anybasis_params['e1'] = Sun-Corona-Syn.Parameter(value=0.01)          # eccentricity of 'per tc secosw sesinw logk'1st planet
...
anybasis_params['per2'] = Sun-Corona-Syn.Parameter(value=42.363011)    # same parameters for 2nd planet ...
anybasis_params['tc2'] = Sun-Corona-Syn.Parameter(value=2082.62516)
anybasis_params['e2'] = Sun-Corona-Syn.Parameter(value=0.01)
anybasis_params['w2'] = Sun-Corona-Syn.Parameter(value=np.pi/2.)
anybasis_params['k2'] = Sun-Corona-Syn.Parameter(value=10.0)

anybasis_params['dvdt'] = Sun-Corona-Syn.Parameter(value=0.0)        # slope
anybasis_params['curv'] = Sun-Corona-Syn.Parameter(value=0.0)         # curvature

# analytically calculate gamma if vary=False and linear=True
anybasis_params['gamma_j'] = Sun-Corona-Syn.Parameter(1.0, vary=False, linear=True)      # velocity zero-point for hires_rj
anybasis_params['jit_j'] = Sun-Corona-Syn.Parameter(value=2.6)        # jitter for hires_rj
...
path = os.path.join(Sun-Corona-Syn.DATADIR, 'epic203771098.csv')
...
priors = [
    ...
    Sun-Corona-Syn.prior.PositiveKPrior( nplanets ),             # Keeps K > 0
    Sun-Corona-Syn.prior.Gaussian('tc1', params['tc1'].value, 0.01), # Gaussian prior on tc1 with center at tc1 and width 0.01 days
    Sun-Corona-Syn.prior.Gaussian('per1', params['per1'].value, 0.01),
    Sun-Corona-Syn.prior.Gaussian('tc2', params['tc2'].value, 0.01),
    Sun-Corona-Syn.prior.Gaussian('per2', params['per2'].value, 0.01),
    Sun-Corona-Syn.prior.HardBounds('jit_j', 0.0, 15.0)
]
